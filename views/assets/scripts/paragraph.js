const containerOtherParagraphs = document.querySelector(
  ".container_paragraphs"
);

let paragraphs = document.querySelectorAll(
  ".container_paragraphs .box_paragraph"
);
...
function removeBoxParagraph() {
  ...
  let boxActionsLastParagraph = lastParagraph.querySelector(
    ".box_actions_paragraph"
  );
  ...
  if (paragraphs.length >= 2) {
    boxActionsParagraph.classList.add(
      "box_actions_paragraph",
      "new_box_paragraph"
    );
...
function addActionButtons() {
  let buttonsRemoveParagraph = document.querySelectorAll(
    ".btn_remove_paragraph"
  );
...
function reorderParagraphs() {
  ...
  paragraphs.forEach((paragraph, indexCurrentParagraph) => {
    ...
    textareaParagraph.setAttribute(
      "name",
      `contentParagraph-${indexCurrentParagraph}`
    );
  });
}
