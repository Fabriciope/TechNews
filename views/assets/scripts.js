...
liDropDown.onclick=toggleActionsProfile;let btnAddParagraph=document.querySelectorAll(".box_add_paragraph");const containerParagraphs=document.querySelector(".container_paragraphs");let btnRemoveParagraph=[];let currentParagraph=1;btnAddParagraph.forEach(btn=>{btn.addEventListener("click",addNewParagraph)});function addNewParagraph(){currentParagraph++;removeActionButtons();createBoxNewParagraph()}
function createBoxNewParagraph(){let boxParagraph=document.createElement("div");boxParagraph.classList.add("box_paragraph");boxParagraph.innerHTML=`
    ...
    `;containerParagraphs.appendChild(boxParagraph);addActionButtons()}
function removeBoxParagraph(){currentParagraph--;let boxParagraphRemove=this.parentElement.parentElement;boxParagraphRemove.remove();let paragraphs=document.querySelectorAll(".box_paragraph");let lastParagraph=paragraphs[paragraphs.length-1];let boxActionsParagraph=document.createElement("div");if(paragraphs.length>=2){boxActionsParagraph.classList.add("box_actions_paragraph","new_box_paragraph");boxActionsParagraph.innerHTML=`
        <div class="box_remove_paragraph">
            <span class="icon_remove_paragraph">
                <i class="fa-solid fa-trash-can"></i>
            </span>
...
function removeActionButtons(){btnAddParagraph.forEach(boxAdd=>{boxAdd.remove()})
btnAddParagraph=document.querySelectorAll(".box_add_paragraph");btnRemoveParagraph.forEach(btn=>{btn.remove()});btnRemoveParagraph=document.querySelectorAll(".box_remove_paragraph")}
function addActionButtons(){btnRemoveParagraph=document.querySelectorAll(".box_remove_paragraph");btnRemoveParagraph.forEach(btn=>{btn.addEventListener("click",removeBoxParagraph)});btnAddParagraph=document.querySelectorAll(".box_add_paragraph");btnAddParagraph.forEach(btn=>{btn.addEventListener("click",addNewParagraph)})}