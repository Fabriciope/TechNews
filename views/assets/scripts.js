Object.values(document.forms).forEach(form=>{if(form.classList.contains('formAjax')){form.addEventListener('submit',async(event)=>{event.preventDefault();const formData=new FormData(form);try{fetch(form.action,{method:form.method,body:formData}).then(data=>{return data.json()}).then(response=>{if(response.redirect){location.href=response.redirect}
